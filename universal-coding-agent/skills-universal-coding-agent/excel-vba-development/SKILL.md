...
description: Microsoft Excel VBA Development. Use this skill when creating VBA solutions for Microsoft Excel, including macro development, Excel automation, workbook/worksheet manipulation, and building enterprise-level Excel applications.
---

# Microsoft Excel VBA Development Guide

## Purpose
This skill provides comprehensive guidance for developing VBA solutions for Microsoft Excel. It covers architectural principles, coding standards, best practices, and development methodologies for creating robust, performant, and scalable enterprise-level Excel applications.
...
- **Architecture**: Apply clean architecture principles (modularity, loose coupling, high cohesion) and relevant design patterns
- **Performance**: Design with optimization in mind (minimize DOM calls, use arrays, efficient resource management)
- **Reliability**: Create robust solutions with comprehensive error handling, recovery mechanisms, and protection against invalid data
- **Maintainability**: Develop self-documenting, clean code with clear structure and logical naming
...
### Module Structure
Each module should include a header with:
- Module name
...
- **Response Handling**: Always check HTTP response status
- **Timeouts and Retries**: Set reasonable timeouts, implement retry logic
- **Rate Limiting**: Implement rate limit compliance mechanisms
- **HTTP Libraries**: Use `MSXML2.XMLHTTP60` or `WinHttp.WinHttpRequest.5.1`
- **Security (API Keys)**: Never store keys, passwords, or tokens directly in code

## Security and Data Validation
- **Input Validation**: Always validate user input
- **Restricted Operations**: Never use `Evaluate()` with user input
...
- **Language Standardization**: All user interface elements visible to the end user must be in Russian (unless specified otherwise)
